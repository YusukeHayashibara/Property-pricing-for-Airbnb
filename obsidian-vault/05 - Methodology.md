...
- #decision 2026-09-07: **IPTU-SP** (São Paulo municipal IPTU / cadastro predial, `data/raw/IPTU-SP/`) is the cost source, replacing Rio's [[ITBI]] / Data.Rio. EDA in `notebooks/iptu_analysis.ipynb`. The [[ITBI]] note still needs updating.
- #decision 2026-09-07: **SSP-SP** crime data (`data/raw/ISP/`, 12 monthly files for 2025) is the risk source, replacing Rio's [[ISP]] / ISP-RJ. EDA in `notebooks/crime_analysis.ipynb`. The [[ISP]] note still needs updating.
- #decision 2026-09-07: the common spatial unit for the whole pipeline is the **distrito** (96 in São Paulo). Every source is normalized to a `distrito` key; the investment score is produced at `distrito × property-profile`. Full rationale, join keys, execution steps, and gaps: [[09 - Spatial Aggregation]].
