...
- 2026-08-20: added "Final candidate scoring" step (region+profile scores → real buy candidates). Reason: no cost source (ITBI/IPTU/FipeZAP/SECOVI) contains active for-sale listings — only a listings-portal snapshot can. Scope: one-off collection at the end of the pipeline, not continuous scraping. #decision
- 2026-08-20: project scope migrated from Rio de Janeiro to São Paulo. Trigger: the Inside Airbnb discovery notebook (`notebooks/airbnb_analysis.ipynb`) found the loaded snapshot's neighborhoods and lat/lon bounding box match São Paulo, not Rio. Impact: ITBI/Data.Rio, ISP-RJ, and the IPTU/Cadastro candidate need São Paulo equivalents — IPTU/Cadastro Predial already identified, see [[IPTU - Cadastro Predial (GeoSampa - SP)]]. Pending: README, Project Overview, and remaining RJ-scoped source docs still need updating to São Paulo (not scheduled). #decision
